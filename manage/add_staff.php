...
if (isset($_POST["user"], $_POST['pass'], $_POST['fname'], $_POST['lname'])) {
    ...
    if ($stmt_check_id_user->num_rows > 0) {
        echo '<script>alert("เกิดข้อผิดพลาดบางอย่าง โปรดลองใหม่อีกครั้ง"); window.location.href="../manage_staff.php";</script>';
    } else {
        ...
        if ($stmt_check_user->num_rows > 0) {
            echo '<script>alert("มี user นี้อยู่ในระบบอยู่แล้ว โปรดลองใหม่อีกครั้ง"); window.location.href="../manage_staff.php";</script>';
        } else {
            ...
            if (isset($_POST["shop_name"])) {
                ...
                $sql_shop = "INSERT INTO shop (id_user, shop_name, status) VALUES (?, ?, 1)";
                $stmt_shop = $conn->prepare($sql_shop);
                $stmt_shop->bind_param("ds", $id_user, $shop_name);
                $stmt_shop_success = $stmt_shop->execute(); // แก้ไขตรงนี้เป็น $stmt_shop

                // Using prepared statement to prevent SQL injection
                $sql_user = "INSERT INTO user (id_user, user, pass, perm, fname , lname, status) VALUES (?, ?, ?, 1, ?, ?, 1)";
                $stmt = $conn->prepare($sql_user);
                $stmt->bind_param("dssss", $id_user, $user, $hashed_pass, $fname, $lname);
                $stmt_success = $stmt->execute();

                // Check if the query was successful
                if ($stmt_success && $stmt_shop_success) {
                ...
                // Using prepared statement to prevent SQL injection
                $sql_user = "INSERT INTO user (id_user, user, pass, perm, fname , lname, status) VALUES (?, ?, ?, 3, ?, ?, 1)";
                $stmt = $conn->prepare($sql_user);
                $stmt->bind_param("dssss", $id_user, $user, $hashed_pass, $fname, $lname);
                $stmt_success = $stmt->execute();
